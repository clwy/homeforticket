...
public class DistributorInfoMessageParser extends AbstractParser<DistributorInfoMessage> {

    @Override
    public DistributorInfoMessage parseInner(String parser) throws Exception {

        DistributorInfoMessage message = new DistributorInfoMessage();
        ...
        try {
            ...
            List<DistributorInfo> infos = new ArrayList<DistributorInfo>();
            String list = getString(jb, "list");
            JSONArray listJson = new JSONArray(list);
            for (int i = 0, size = listJson.length(); i < size; i++) {
                infos.add(new DistributorInfoParser().parseInner(listJson.getString(i)));
            }
